var CACHE='khaios-v2';
var SHELL=['./','./index.html','./app.html','./manifest.json','./icon.svg'];
...
self.addEventListener('fetch',function(e){
  ...
  if(req.mode==='navigate'||(req.headers.get('accept')||'').indexOf('text/html')>-1){
    e.respondWith(fetch(req).then(function(res){var copy=res.clone();caches.open(CACHE).then(function(c){c.put(req,copy);});return res;}).catch(function(){return caches.match(req).then(function(r){return r||caches.match('./index.html').then(function(x){return x||caches.match('./');});});}));
    return;
  }
